...
ruta_input = BASE_DIR /"Inputs"/"Process data"/"Information_2019_2025"/"2023"
...
R1=os.listdir()# Crear DataFrame vacío
Consolidado = pd.DataFrame()
i=0
for r1_seleccionado in R1:
    try:
        temporal = pd.read_csv(r1_seleccionado, sep=";",on_bad_lines='skip',encoding='latin1')
    except:
        try:
            temporal = pd.read_csv(r1_seleccionado, sep=",",on_bad_lines='skip',encoding='latin1')
        except:
    ...
    if len(temporal.columns) == 1:
        ...
        for sep in [';', ',', '|', '\t']:
            if sep in temporal.columns[0]:
                ...
                break
        if sep_detectado:
            temporal = pd.read_csv(r1_seleccionado, sep=sep_detectado, on_bad_lines='skip',encoding='latin1')
...
ruta_consolidado = BASE_DIR /"Outputs"/"2023"/"R1"
